...
def test_clean_case_auto_approves_without_review() -> None:
    route, needs_approval = choose_route([], confidence=0.96)
...
def test_low_confidence_clean_case_requires_approval() -> None:
    route, needs_approval = choose_route([], confidence=0.70)
...
def test_high_severity_case_escalates_to_legal() -> None:
    route, needs_approval = choose_route(["high"], confidence=0.90)
...
def test_requested_specialist_route_is_preserved() -> None:
    route, needs_approval = choose_route(["medium"], requested_route="finance", confidence=0.90)
